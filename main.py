...
import requests, signal, os, base64, subprocess, threading, sys
...
def take_photo_and_print_poem():
  ...
  directory = os.path.expanduser('~') +  "/poetry-camera-rpi/"
...
################################
def get_poem_format():
  poem_format = '6 line free verse. Do not rhyme. DO NOT EXCEED LINES.'
  return poem_format
...
###########################
# SAVE POEM AND IMAGE TO INSTAGRAM
###########################
def insta_post(poem):
 cl = Client()
 ...
 photo = directory + "image.jpg"
 poem_bottom = '\n\n @Poetry.Camera'
 #draw = ImageDraw.Draw(photo)
 #draw.text((250, 250), poem, fill =(0, 0, 0))
 #img.save(photo)
 media = cl.photo_upload(photo,poem + poem_bottom)
